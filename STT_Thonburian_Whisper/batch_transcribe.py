...
INPUT_DIR = BASE_DIR / "additional_data_2"
...
# OUTPUT_DIR = BASE_DIR / "additional_data_2" / "normal_phone_transcribed"
OUTPUT_DIR = BASE_DIR / "additional_data_2" / "scam_phone_transcribed"
...
audio_files = list(INPUT_DIR.glob("*.wav")) + list(INPUT_DIR.glob("*.mp3")) + list(INPUT_DIR.glob("*.mp4"))
...
for index, audio_file in enumerate(audio_files, 1):
    ...
    print(f"[{index}/{len(audio_files)}] Processing: {str(audio_file)} into {str(output_path)}...")
